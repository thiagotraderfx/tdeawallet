...
import { FaucetFormSchema, type FaucetFormState } from '@/lib/schema';
import { buildClaimResult } from '@/lib/claim-utils';
...
function getFaucetAccount(): algosdk.Account {
  ...
  if (!mnemonic) {
    console.error(
      'CRITICAL ERROR: ALGO_MNEMONIC environment variable is not set.'
    );
  ...
  try {
    ...
    throw new Error(
      'Server configuration is invalid. Cannot initialize faucet account.'
    );
  }
}
...
export async function claimAlgo(
  ...
  console.info(`[claimAlgo][START]`, {
    requestId,
    to: recipientAddressValue,
  ...
  try {
    ...
    if (balance < BigInt(FAUCET_AMOUNTS.userRequest + ALGORAND_MIN_TX_FEE)) {
      console.error(
        `CRITICAL: Faucet has insufficient funds. Balance: ${balance}, Required: ${
          FAUCET_AMOUNTS.userRequest + ALGORAND_MIN_TX_FEE
        }`
      );
      const result = buildClaimResult({
        success: false,
    ...
    const txn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
      ...
      note: new Uint8Array(
        Buffer.from(
          '🌟 TdeA Faucet envía 1️⃣ ALGO ✅️ Utilízalo como quieras 🔑'
        )
      ),
    });
    ...
    const result = buildClaimResult({
      ...
      txId: txId,
      explorerUrl: 'https://testnet-explorer.algonode.io/',
    });
    ...
    console.error('[claimAlgo][UNEXPECTED_ERROR]', {
      requestId,
    ...
    if (error.message.toLowerCase().includes('fetch')) {
      const result = buildClaimResult({
        success: false,
        message:
          'Error de red al conectar con el nodo de Algorand. Por favor, inténtalo más tarde.',
        code: 'NETWORK_ERROR',
      });
      console.info(`[claimAlgo][RETURN]`, {
        requestId,
